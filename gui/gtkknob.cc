...
#define GTK_OVERDRIVER_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_OVERDRIVER_KNOB, GtkOverdriverKnobPrivate))

G_DEFINE_TYPE(GtkOverdriverKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkOverdriverKnobPrivate GtkOverdriverKnobPrivate;

struct _GtkOverdriverKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkOverdriverKnobPrivate *priv = GTK_OVERDRIVER_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkOverdriverKnob *knob = GTK_OVERDRIVER_KNOB(widget);
...
{
    g_assert(GTK_IS_OVERDRIVER_KNOB(widget));
    GtkOverdriverKnob *knob = GTK_OVERDRIVER_KNOB(widget);
    GtkOverdriverKnobPrivate *priv = GTK_OVERDRIVER_KNOB_GET_PRIVATE(knob);
...
{
    GtkOverdriverKnob *knob = GTK_OVERDRIVER_KNOB(object);
...
/****************************************************************
 ** init the GtkOverdriverKnobClass
 */

static void gtk_knob_class_init (GtkOverdriverKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkOverdriverKnobPrivate));
}
...
static void gtk_knob_init (GtkOverdriverKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_OVERDRIVER_KNOB, NULL ));
